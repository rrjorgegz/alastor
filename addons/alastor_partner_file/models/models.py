...
class Partner(models.Model):
    ...
    client_files_count = fields.Integer(
        compute=_get_client_files_count, string="Fichas de cliente"
    )
...
class ClientFileContactInvoice(models.Model):
    ...
    client_file_id = fields.Many2one(
        "res.partner.file", string="Client File", ondelete="cascade"
    )
...
class ClientFile(models.Model):
    ...
    def name_get(self):
        ...
        for item in self:
            result.append(
                (
                    item.id,
                    (item.initial_date and (item.initial_date) + " -> " or " ")
                    + (item.final_date and (item.final_date) or " "),
                )
            )
        return result
    ...
    represent_name = fields.Char(
        "Nombre y apellidos del representante para firmar contratos"
    )
    ...
    state = fields.Selection(
        [("draft", "Borrador"), ("active", "Activa"), ("cancel", "Cancelada")],
        string="Estado",
        default="draft",
    )
    invoice_authorized_ids = fields.One2many(
        "res.partner.file.contact.invoice",
        "client_file_id",
        string="Personas autorizadas a solicitar servicios y firmar facturas",
    )
    reconciliation_authorized_ids = fields.One2many(
        "res.partner.file.contact.reconciliation",
        "client_file_id",
        string="Personas autorizadas a firmar conciliaciones",
    )
    ...
    def case_activate(self):
        ...
        if not self.initial_date:
            raise exceptions.Warning(
                _(
                    "Debe proveer una fecha de inicio antes de activar la ficha de cliente"
                )
            )
    ...
    def case_cancel(self):
        if not self.final_date:
            raise exceptions.Warning(
                _("Debe proveer una fecha de fin antes de cancelar la ficha de cliente")
            )
    ...
    def unlink(self):
        if self.state not in ("draft",):
            raise exceptions.Warning(
                _("Solo se pueden eliminar las fichas de cliente en estado borrador")
            )
    ...
    def copy(self, default=None):
        ...
        for ia in self.invoice_authorized_ids:
            vals = {
                "client_file_id": res.id,
                "name": ia.name,
                "id_number": ia.id_number,
                "job": ia.job,
            }
