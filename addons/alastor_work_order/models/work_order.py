...
class WorkOrder(models.Model):
    ...
    number = fields.Char(
        ...
        index=True,
        readonly=True,
        default=lambda s: s.env["ir.sequence"].next_by_code("wor.ord"),
        help="Número único del documento, " "calculado automáticamente.",
    )
    ...
    ref_contract_id = fields.Many2one(
        "partner.sale.contract", "Contrato referente", required=True, store=True
    )
        ...
        if self.ref_contract_id:
            if self.ref_contract_id.state not in ("approval", "active"):
                raise exceptions.Warning(
                    _("El contrato referente debe estar aprobado o activo")
                )
        return super(WorkOrder, self).write(vals)

    @api.model
    def create(self, vals):
        contract = self.env["partner.sale.contract"].search(
            [("id", "=", vals.get("ref_contract_id"))]
        )
        if contract.state not in ("approval", "active"):
            raise exceptions.Warning(
                _("El contrato referente está vencido o no está activo")
            )
...
class WorkActivity(models.Model):
    ...
    work_order_id = fields.Many2one(
        "work.order", ondelete="cascade", string="Orden de trabajo", required=True
    )
...
class CostCenter(models.Model):
    ...
    _sql_constraints = [
        (
            "name",
            "unique(name)",
            "No puede existir centros de costos con el mismo nombre",
        )
    ]
