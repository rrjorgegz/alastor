...
y aun no se ha planificado su aprobación.""" % str(
            ...
            if i.state == "scheduled":
                ...
                if delta < -2:
                    i.is_late = True
                    i.warning_msg = """Este documento estaba planificado
para hace %s días y aun no se ha aprobado o rechazado.""" % str(
                        abs(delta)
                    )
    ...
    def _html_info(self):
        ...
        if self.state == "scheduled" or self.state == "received":
            delta = relativedelta(
                datetime.today(), datetime.strptime(self.reception_date, "%Y-%m-%d")
            )
            data += (
                ", "
                + _("creado hace ")
        ...
        self.html_info = (
            ...
            + "/web#id="
            + str(self.id)
            + '&view_type=form&model=partner.purchase.supplement">ver</a>'
        )
    ...
    number = fields.Char(
        ...
        "calculado automaticamente cuando el documento es aprobado.",
    )
    ...
    supplier_ref = fields.Char(
        "Referencia Proveedor", size=256, help="Supplier Reference", required=True
    )
    ...
    add_currency_id = fields.Many2one(
        "res.currency", "Moneda Adicional", related="company_id.add_currency_id"
    )
    ...
    def create(self, vals):
        ...
        if frame_parent_id:
            frame_parent = self.env["partner.purchase.frame.contract"].search(
                [("id", "=", frame_parent_id)]
            )
            ...
            if vals.get("reception_date") < frame_parent.reception_date.strftime(
                "%Y-%m-%d"
            ):
                raise exceptions.Warning(
                    _(
                        """La fecha de recepción no
puede ser menor que la fecha de recepción del padre"""
                    )
                )

        parent_id = vals.get("parent_id", False)
        if parent_id:
            parent = self.env["partner.purchase.contract"].search(
                [("id", "=", parent_id)]
            )
...
ser hijos de un Contrato Marco y de un Contrato a la vez"""
    ...
    def write(self, vals):
        ...
        self._validate_reception_date(reception_date)
        ...
        self._validate_contract(frame_parent, reception_date, _("contrato marco"))
        self._validate_contract(parent, reception_date, _("contrato padre"))
    ...
    def _validate_contract(self, contract, reception_date, contract_type_label):
        if contract:
            if contract.state not in ("approved", "active"):
                raise exceptions.Warning(
                    _("El %s debe estar aprobado o activo") % contract_type_label
                )
            contract_reception_date = contract.reception_date.strftime("%Y-%m-%d")
            if reception_date < contract_reception_date:
                raise exceptions.Warning(
                    _("La fecha de recepción no puede ser menor que la del %s")
                    % contract_type_label
                )
    ...
    def _get_activation_date(self, vals):
        ...
        if activation_date:
            return fields.Date.from_string(activation_date)
    ...
    def _compute_expiration_date(
        ...
        if term_type == "until":
            ...
            return False
        elif term_type == "fixed" and activation_date and term and term_uom:
            if term_uom == "year":
                return activation_date + relativedelta(years=int(term))
            elif term_uom == "month":
                return activation_date + relativedelta(months=int(term))
            elif term_uom == "week":
                return activation_date + relativedelta(weeks=int(term))
            elif term_uom == "day":
                return activation_date + relativedelta(days=int(term))
        return None
    ...
    def case_scheduled(self):
        ...
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_supplement_scheduled",
    ...
    def case_rejected(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_supplement_rejected",
            raise_if_not_found=False,
        )
    ...
    def case_approved(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_supplement_approved",
            raise_if_not_found=False,
        )
...
puede ser menor que la fecha de aprobación del padre"""
    ...
    def case_active(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_supplement_actived",
            raise_if_not_found=False,
        )

        for supplement in self:
            if supplement.state not in ("approved", "resent"):
                continue
    ...
    def _validate_activation_requirements(self, supplement):
        ...
        term_type = supplement.term_type
        if term_type == "until" and not supplement.term_date:
            raise exceptions.Warning(
        ...
        if (
            ...
            raise exceptions.Warning(
                _(
                    ...
                    ser menor que la fecha de activación"""
                )
            )

    def _handle_parent_updates(self, supplement):
        if supplement.update_parent_validity:
    ...
    def _update_parent_expiration(self, supplement, parent):
        ...
        cause = supplement.supplement_cause
        parent_exp_date = parent.expiration_date

        if cause == "extension" and parent_exp_date < supplement.expiration_date:
            parent.write({"expiration_date": supplement.expiration_date})
        elif cause == "reduction" and parent_exp_date > supplement.expiration_date:
            parent.write({"expiration_date": supplement.expiration_date})
    ...
    def case_close(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_supplement_done",
            raise_if_not_found=False,
        )
    ...
    def name_search(
        ...
        if context is None:
            context = {}
        ids = self.env["partner.purchase.supplement"].search(
