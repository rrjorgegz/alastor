...
class PartnerPurchaseFrameContract(models.Model):
    ...
    def _html_info(self):
        ...
        self.html_info = (
            ...
            + "/web#id="
            + str(self.id)
            + '&view_type=form&model=partner.purchase.frame.contract">ver</a>'
        )
    ...
    number = fields.Char(
        ...
        "calculado automáticamente al aprobar el documento.",
    )
    ...
    contract_ids = fields.One2many(
        "partner.purchase.contract", "parent_id", "Contratos"
    )
    contract_count = fields.Integer(
        compute=_get_contract_count, string="Número de contratos"
    )
    supplement_ids = fields.One2many(
        "partner.purchase.supplement", "frame_parent_id", "Suplementos"
    )
    ...
    def unlink(self):
        if self.state != "received":
            raise exceptions.Warning(
                _(
                    "No puede eliminar un contrato marco que no esté en estado 'Recibido'."
                )
            )
    ...
    def case_scheduled(self):
        ...
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_frame_contract_scheduled",
    ...
    def case_rejected(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_frame_contract_rejected",
            raise_if_not_found=False,
        )
    ...
    def case_approved(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_frame_contract_approved",
            raise_if_not_found=False,
        )
            ...
            if it.state in ("received", "scheduled"):
                ...
                if it.number == "" or it.number == "/":
                    seq = self.env["ir.sequence"].next_by_code(
                        "partner.frame.contract.purchases"
                    )
                    it.write({"number": seq})
    ...
    def case_active(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_frame_contract_actived",
            raise_if_not_found=False,
        )
    ...
    def case_close(self):
        template = self.env.ref(
            "alastor_partner_contract.email_template_purchase_frame_contract_closed",
            raise_if_not_found=False,
        )
        ...
        for it in self:
            if it.state == "active":
                ...
                list_contracts = self.env["partner.purchase.contract"].search(
                    [
                        ("parent_id", "=", it.id),
                        ("state", "not in", ["rejected", "done"]),
                    ]
                ...
                if len(list_contracts) > 0:
                    raise exceptions.Warning(
                        _(
                            "No puede cerrar este contrato marco porque tiene contratos hijos"
                        )
                    )
    ...
    def name_search(
        ...
        if context is None:
            context = {}
        ids = self.env["partner.purchase.frame.contract"].search(
