...
class SaleDocs(models.TransientModel):
    ...
    company_ids = fields.Many2many(
        "res.company", "sale_docs_res_company_rel", string="Entidades"
    )
    ...
    def print_report(self, context=None):
        ...
        if len(companies):
            frame_contract_ids = self.env["partner.sale.frame.contract"].search(
                [("state", "in", states), ("company_id", "in", companies)],
                order="emission_date asc",
                context=context,
            )
            contract_ids = self.env["partner.sale.contract"].search(
                [("state", "in", states), ("company_id", "in", companies)],
                order="emission_date asc",
                context=context,
            )
            supplement_ids = self.env["partner.sale.supplement"].search(
                [("state", "in", states), ("company_id", "in", companies)],
                order="emission_date asc",
                context=context,
            )
            ...
            frame_contract_ids = self.env["partner.sale.frame.contract"].search(
                [("state", "in", states)], order="emission_date asc", context=context
            )
            contract_ids = self.env["partner.sale.contract"].search(
                [("state", "in", states)], order="emission_date asc", context=context
            )
        ...
        return {
            "report_name": "partner_contract.report_sale_docs",
