...
class PurchaseDocs(models.TransientModel):
    ...
    company_ids = fields.Many2many(
        "res.company", "purchase_docs_res_company_rel", string="Companies"
    )
    ...
    def print_report(self):
        ...
        if len(companies):
            frame_contract_ids = self.env["partner.purchase.frame.contract"].search(
                [("state", "in", states), ("company_id", "in", companies)],
                order="reception_date asc",
            )
            contract_ids = self.env["partner.purchase.contract"].search(
                [("state", "in", states), ("company_id", "in", companies)],
                order="reception_date asc",
            )
            supplement_ids = self.env["partner.purchase.supplement"].search(
                [("state", "in", states), ("company_id", "in", companies)],
                order="reception_date asc",
            )
        else:
            frame_contract_ids = self.env["partner.purchase.frame.contract"].search(
                [("state", "in", states)], order="reception_date asc"
            )
            contract_ids = self.env["partner.purchase.contract"].search(
                [("state", "in", states)], order="reception_date asc"
            )
        ...
        return (
            self.env.ref("alastor_partner_contract.action_report_purchase_docs")
