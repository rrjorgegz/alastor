...
class PurchaseFrameContract(models.Model):
    ...
    def _html_info(self):
        ...
        if self.state == "approved":
            ...
            data += ", " + _("aprobado hace ") + str(delta) + _(" días")

        web_base_url = self.env["ir.config_parameter"].get_param("web.base.url")
        self.html_info = data + ' <a href="' + str(web_base_url) + "/web#id=" + str(self.id) + '&view_type=form&model=partner.purchase.frame.contract">ver</a>'

    name = fields.Char("Nombre", required=True, index=True, readonly=True, states={"received": [("readonly", False)], "scheduled": [("readonly", False)]})
    user_id = fields.Many2one("res.users", "Usuario", help="User of the contract.", readonly=True, default=lambda s: s.env.user.id)
    ...
    number = fields.Char(
        ...
        default="/",
        help="Número único del documento, " "calculado automáticamente al aprobar el documento.",
    )
    ...
    close_date = fields.Date(
        ...
        states={
            ...
            "resent": [("invisible", True), ("readonly", True)],
    ...
    contract_ids = fields.One2many("partner.purchase.contract", "parent_id", "Contratos")
    contract_count = fields.Integer(compute=_get_contract_count, string="Número de contratos")
    supplement_ids = fields.One2many("partner.purchase.supplement", "frame_parent_id", "Suplementos")
    ...
    def unlink(self):
        if self.state != "received":
            raise exceptions.Warning(_("No puede eliminar un contrato marco que no esté en estado 'Recibido'."))
    ...
    def case_scheduled(self):
        self.ensure_one()
        template = self.env.ref("alastor_partner_contract.email_template_purchase_frame_contract_scheduled", raise_if_not_found=False)
    ...
    def case_rejected(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_frame_contract_rejected", raise_if_not_found=False)
    ...
    def case_approved(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_frame_contract_approved", raise_if_not_found=False)
        ...
        for it in self:
            if it.state in ("received", "scheduled"):
                ...
                if it.number == "" or it.number == "/":
                    seq = self.env["ir.sequence"].next_by_code("partner.frame.contract.purchases")
                    it.write({"number": seq})
    ...
    def case_active(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_frame_contract_actived", raise_if_not_found=False)
    ...
    def case_close(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_frame_contract_closed", raise_if_not_found=False)
        ...
        for it in self:
            if it.state == "active":
                ...
                list_contracts = self.env["partner.purchase.contract"].search([("parent_id", "=", it.id), ("state", "not in", ["rejected", "done"])])
                if len(list_contracts) > 0:
                    raise exceptions.Warning(_("No puede cerrar este contrato marco porque tiene contratos hijos"))

                message = _("El contrato marco '%s' ha sido cerrado.") % it.name
                it.message_post(body=message)
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100, context={}):
        ids = self.env["partner.purchase.frame.contract"].search(["|", ("number", operator, name), ("name", operator, name)] + args)
        return ids.name_get()
