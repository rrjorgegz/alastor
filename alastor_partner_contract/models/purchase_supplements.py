...
AVAILABLE_STATES = [
    ("received", "Recibido"),  # we receive a proforma (1 or 2 days)
...
class PurchaseSupplement(models.Model):
    ...
    is_late = fields.Boolean("Is Late", compute=get_warnings)
    warning_msg = fields.Text("Warnings", compute=get_warnings)

    def _html_info(self):
        data = self.name + ", " + _("number: ") + self.number + ", " + _("supplier: ") + self.partner_id.name
        ...
        if self.state == "scheduled" or self.state == "received":
            delta = relativedelta(datetime.today(), datetime.strptime(self.reception_date, "%Y-%m-%d"))
        ...
        self.html_info = data + ' <a href="' + str(web_base_url) + "/web#id=" + str(self.id) + '&view_type=form&model=partner.purchase.supplement">ver</a>'
    ...
    number = fields.Char(
        ...
        default="/",
        help="Número único del documento, " "calculado automaticamente cuando el documento es aprobado.",
    )
    ...
    supplier_ref = fields.Char("Referencia Proveedor", size=256, help="Supplier Reference", required=True)
    ...
    add_currency_id = fields.Many2one("res.currency", "Moneda Adicional", related="company_id.add_currency_id")
    ...
    frame_parent_id = fields.Many2one("partner.purchase.frame.contract", "Contrato marco padre", readonly=True, states={"received": [("readonly", False)], "scheduled": [("readonly", False)]})
    parent_id = fields.Many2one("partner.purchase.contract", "Contrato padre", readonly=True, states={"received": [("readonly", False)], "scheduled": [("readonly", False)]})
    ...
    update_parent_validity = fields.Boolean(
        "Actualizar validez del padre", readonly=True, default=False, required=False, states={"draft": [("readonly", False)], "scheduled": [("readonly", False)], "approved": [("readonly", False)]}
    )
    ...
    def unlink(self):
        if self.state != "received":
            raise exceptions.Warning(_("No puede eliminar un suplemento sino está en estado Recibido."))
    ...
    def write(self, vals):
        ...
        if vals.get("reception_date", self.reception_date.strftime("%Y-%m-%d")) > date.today().strftime("%Y-%m-%d"):
            raise exceptions.Warning(_("La fecha de recepción no puede ser mayor que hoy"))
        frame_parent = self.env["partner.purchase.frame.contract"].search([("id", "=", vals.get("frame_parent_id", self.frame_parent_id.id))])
        if frame_parent:
            if frame_parent.state not in ("approved", "active"):
                raise exceptions.Warning(_("El contrato padre debe estar aprobado o activo"))
            if vals.get("reception_date", self.reception_date.strftime("%Y-%m-%d")) < frame_parent.reception_date.strftime("%Y-%m-%d"):
                raise exceptions.Warning(_("La fecha de recepción no puede ser menor que la fecha de recepción del padre"))

        parent = self.env["partner.purchase.contract"].search([("id", "=", vals.get("parent_id", self.parent_id.id))])
        if parent:
            if parent.state not in ("approved", "active"):
                raise exceptions.Warning(_("El contrato padre debe estar aprobado o activo"))
            if vals.get("reception_date", self.reception_date.strftime("%Y-%m-%d")) < parent.reception_date.strftime("%Y-%m-%d"):
                raise exceptions.Warning(_("La fecha de recepción no puede ser menor que la fecha de recepción del padre"))
    ...
    def case_scheduled(self):
        self.ensure_one()
        template = self.env.ref("alastor_partner_contract.email_template_purchase_supplement_scheduled", raise_if_not_found=False)
    ...
    def case_rejected(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_supplement_rejected", raise_if_not_found=False)
    ...
    def case_approved(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_supplement_approved", raise_if_not_found=False)
        ...
        for it in self:
            if it.state in ("received", "scheduled"):
                ...
                if it.number == "" or it.number == "/":
                    seq = self.env["ir.sequence"].next_by_code("partner.supplement.purchases")
                    it.write({"number": seq})
    ...
    def case_active(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_supplement_actived", raise_if_not_found=False)
    ...
    def case_close(self):
        template = self.env.ref("alastor_partner_contract.email_template_purchase_supplement_done", raise_if_not_found=False)
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100, context={}):
        ids = self.env["partner.purchase.supplement"].search(["|", ("number", operator, name), ("name", operator, name)] + args)
        return ids.name_get()
