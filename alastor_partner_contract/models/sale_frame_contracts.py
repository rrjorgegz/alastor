...
class SaleFrameContract(models.Model):
    """Sale Frame Contract"""

    _name = "partner.sale.frame.contract"
    _description = "Sale Frame Contract"
    ...
    def _html_info(self):
        ...
        self.html_info = data + ' <a href="' + str(web_base_url) + "/web#id=" + str(self.id) + '&view_type=form&model=partner.sale.frame.contract">ver</a>'

    name = fields.Char("Nombre", required=True, index=True, readonly=True, states={"draft": [("readonly", False)], "proform": [("readonly", False)]})
    user_id = fields.Many2one("res.users", "Usuario", readonly=True, default=lambda s: s.env.user.id)
    ...
    number = fields.Char(
        ...
        default="/",
        help="Número único del documento, " "calculado automáticamente cuando el documento es aprobado.",
    )
    ...
    client_ref = fields.Char("Referencia cliente", size=256, help="Referencia del cliente")
    ...
    contract_ids = fields.One2many("partner.sale.contract", "parent_id", "Contratos")
    contract_count = fields.Integer(compute=_get_contract_count, string="Número de contratos")
    supplement_ids = fields.One2many("partner.sale.supplement", "frame_parent_id", "Suplementos")
    ...
    def create(self, vals):
        ...
        return super(SaleFrameContract, self).create(vals)

    def unlink(self):
        if self.state != "draft":
            raise exceptions.Warning(_("No puede eliminar un contrato marco que no esté en estado Borrador"))

        return super(SaleFrameContract, self).unlink()
        ...
        if self.state == "active" and self.term_action == "extend":
            if self.term_type == "fixed":
                if self.expiration_date and self.term and self.term_uom:
                    ...
                    message = _("El contrato marco '%s' ha sido extendido hasta %s.") % (self.name, new_expiration_date.strftime("%Y-%m-%d"))
    ...
    def write(self, vals):
        ...
        if self.state in ("active", "done", "rejected"):
            return super(SaleFrameContract, self).write(vals)
    ...
    def case_proform(self):
        template = self.env.ref("alastor_partner_contract.email_template_sale_frame_contract_proform", raise_if_not_found=False)
        for it in self:
            if it.state == "draft":
    ...
    def case_approved(self):
        template = self.env.ref("alastor_partner_contract.email_template_sale_frame_contract_approved", raise_if_not_found=False)
        for it in self:
            if it.state == "proform":
                ...
                if it.number == "" or self.number == "/":
                    seq = self.env["ir.sequence"].next_by_code("partner.frame.contract.sales")
    ...
    def case_active(self):
        template = self.env.ref("alastor_partner_contract.email_template_sale_frame_contract_active", raise_if_not_found=False)
        for it in self:
    ...
    def case_close(self):
        template = self.env.ref("alastor_partner_contract.email_template_sale_frame_contract_done", raise_if_not_found=False)
        for it in self:
            if it.state == "active":
                list_supplements = self.env["partner.sale.supplement"].search([("parent_id", "=", self.id), ("state", "not in", ["rejected", "done"])])
                if len(list_supplements) > 0:
                    raise exceptions.Warning(_("No puede cerrar este contrato marco porque tiene suplementos"))

                list_contracts = self.env["partner.sale.contract"].search([("parent_id", "=", it.id), ("state", "not in", ["rejected", "done"])])
                if len(list_contracts) > 0:
                    raise exceptions.Warning(_("No puede cerrar este contrato marco porque tiene contratos"))

                it.write({"close_date": fields.Date.today(), "state": "done"})
                if template:
                    it.with_context(force_send=True).message_post_with_template(template.id)
    ...
    def name_get(self):
        ...
        return super(SaleFrameContract, self).name_get()

    @api.model
    def name_search(self, name="", args=None, operator="ilike", limit=100, context={}):
        ids = self.env["partner.sale.frame.contract"].search(["|", ("number", operator, name), ("name", operator, name)] + args)
        return ids.name_get()
