...
try:
    ...
    raise ImportError("This module needs paramiko to automatically write backups to the FTP through SFTP. " "Please install paramiko on your system. (sudo pip3 install paramiko)")
...
class DbBackup(models.Model):
    ...
    name = fields.Char("Database", required=True, help="Database you want to schedule backups for", default=_get_db_name)
    ...
    days_to_keep = fields.Integer(
        "Remove after x days", help="Choose after how many days the backup should be deleted. For example:\n" "If you fill in 5 the backups will be removed after 5 days.", required=True
    )
    ...
    sftp_password = fields.Char(
        "Password User SFTP Server", help="The password from the user where the SFTP connection should be made with. This " "is the password from the user on the external server."
    )
                ...
                try:
                    ...
                    if rec.send_mail_sftp_fail:
                        try:
                            ...
                            msg = ir_mail_server.build_email(response_mail, [rec.email_to_notify], "Backup from " + rec.host + "(" + rec.sftp_host + ") failed", message)
